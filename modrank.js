...
await pool.query( CREATE TABLE IF NOT EXISTS mod_rank ( user_id TEXT PRIMARY KEY, username TEXT NOT NULL, points INT DEFAULT 0, joined_at TIMESTAMP DEFAULT NOW() ); );

// Create bump_leaderboard table for Disboard bumps await pool.query(` CREATE TABLE IF NOT EXISTS bump_leaderboard ( user_id TEXT PRIMARY KEY, username TEXT NOT NULL, bumps INT DEFAULT 0 ); `); console.log('✅ Tables for mod_rank and bump_leaderboard checked/created.'); 
...
try {
await client.query( INSERT INTO mod_rank (user_id, username, points, joined_at) VALUES ($1, $2, 1, NOW()) ON CONFLICT (user_id) DO UPDATE SET username = EXCLUDED.username, points = mod_rank.points + 1 , [userId, username]);
} finally {
...
await client.query( INSERT INTO bump_leaderboard (user_id, username, bumps) VALUES ($1, $2, 1) ON CONFLICT (user_id) DO UPDATE SET username = EXCLUDED.username, bumps = bump_leaderboard.bumps + 1 , [mentionedUser.id, mentionedUser.username]);

// If the mentioned user is a moderator, award points as well const member = message.guild.members.cache.get(mentionedUser.id); if (member && member.roles.cache.some(role => role.name.toLowerCase() === 'moderator')) { await client.query(` INSERT INTO mod_rank (user_id, username, points, joined_at) VALUES ($1, $2, 3, NOW()) ON CONFLICT (user_id) DO UPDATE SET username = EXCLUDED.username, points = mod_rank.points + 3 `, [mentionedUser.id, mentionedUser.username]); } } finally { client.release(); // Release connection back to the pool } 
...
const client = await pool.connect();
try {
const result = await client.query( SELECT user_id, username, points, joined_at, (DATE_PART('day', NOW() - joined_at) + 1) AS days_as_mod FROM mod_rank WHERE points > 0 ORDER BY points DESC );
...
const client = await pool.connect();
try {
const result = await client.query( SELECT user_id, username, bumps FROM bump_leaderboard ORDER BY bumps DESC );
...
// Call updateModRank when a message is sent by a moderator updateModRank(message.author.id, message.author.username, message.guild); // Call trackBumpingPoints to track bump messages trackBumpingPoints(message); 
